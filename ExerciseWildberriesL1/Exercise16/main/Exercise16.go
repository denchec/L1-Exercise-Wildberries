...
func main() {
	ar := []int{3, 7, 1, 2, 5, 4, -1, 0, 8} // создали массив
	Quicksort(ar)                           // Запустили функцию Quicksort
	fmt.Println(ar)                         // Вывели отсортированный массив
}

func Quicksort(ar []int) {
	if len(ar) <= 1 { // Если длина массива меньше или равна 1 - сортировка не нужна, выводим массив
		return
	}

	split := partition(ar) // Запустили функцию partition для сортировки массива

	Quicksort(ar[:split]) // Повторяем функцию Quicksort сначала в левой части, от переменной split
	Quicksort(ar[split:]) // Повторяем функцию Quicksort сначала в левой части, от переменной split
}
...
func partition(ar []int) int {
	...
	left := 0            // Задаем начало массива
	right := len(ar) - 1 // Определяем конец массива
